...
/**
 * Convert scraped portfolio data into a structured profile JSON payload for AI.
 * @param {Object} data - Scraped portfolio data
 * @returns {Object} Structured profile object
 */
export function buildStructuredProfile(data = {}) {
  const safeArray = (value, max = 30) => (Array.isArray(value) ? value.slice(0, max) : []);

  const projectLinkHealth = safeArray(data.projectLinkHealth, 60);
  const workingProjectLinks = projectLinkHealth.filter((item) => item?.status === 'working').length;
  const brokenProjectLinks = projectLinkHealth.filter((item) => item?.status === 'broken').length;
  ...
  return {
    profile: {
      ...
      bio: safeArray(data.bio, 10),
      experience: safeArray(data.experience, 30),
      technologies: safeArray(data.techStack || data.technologies, 60),
      socialLinks: safeArray(data.socialLinks, 30),
      designLinks: safeArray((data.links || []).filter((l) => /dribbble\.com|behance\.net|figma\.com/i.test(l?.href || '')), 20),
    },
    projects: {
      snippets: safeArray(data.projects, 50),
      links: safeArray(data.projectLinks, 50),
      linkHealth: projectLinkHealth,
      count: safeArray(data.projects, 1000).length,
      linkCount: safeArray(data.projectLinks, 1000).length,
      workingLinks: workingProjectLinks,
      brokenLinks: brokenProjectLinks,
    },
    writing: {
      hasBlogSection: Boolean(data.hasBlogSection),
      blogArticles: safeArray(data.blogArticles, 50),
      paragraphSnippets: safeArray(data.paragraphs, 40),
    },
    github: {
      repositoryLinks: safeArray((data.projectLinks || []).filter((p) => /github\.com|gitlab\.com|bitbucket\.org/i.test(p?.url || '')), 40),
    },
    siteSignals: {
      headings: safeArray(data.headings, 80),
      crawledPages: safeArray(data.crawledPages, 12),
      imageCount: safeArray(data.images, 10000).length,
      linkCount: safeArray(data.links, 10000).length,
